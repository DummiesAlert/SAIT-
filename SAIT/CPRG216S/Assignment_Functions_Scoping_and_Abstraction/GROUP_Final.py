...
def computeRoomArea(): #Couldn't Get it Working Inside the Function but, can Ouside Below
...
def computeRectangleWallsArea():
    ...
    computeRectangleArea(Length, Width, Height)
...
def computeCustomWallsArea(): 
    ...
    for j in range (1, Wall_Value + 1): 
        
        Wall_Length = float(input(f"Enter the Length of Wall {j} in Feet: "))
        Wall_Height = float(input(f"Enter the Width of Wall {j} in Feet: "))
...
while i <= Room_Num: 
    ...
    T_Cost = (T_Square * LABOUR_COST + T_Paint_Cost) * PROFIT_RANGE #Calculate the Total Cost by Multiplying the Paint Cost, the Area, Labour Cost and Overhead 
